...
- Added static (DC worst-case) current inference to the ERC: typed passives link nets with their DC impedance (resistors `1/R`, inductors/ferrites as near-shorts, capacitors open at DC), so a divider from a declared-voltage rail reports its exact static draw with no hand-written current declarations. Inferred currents land as `current_sink_static`/`current_source_static` net properties, count toward the `erc.current_budget` check and feed the layout-level per-branch flow analysis; resistive subnetworks that cannot be solved from declared data raise the new `erc.current_budget.uninferable` warning.
