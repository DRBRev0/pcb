...
`Net(name_or_net=None, voltage=None, impedance=None, signal=None, matched_group=None)` accepts a
positional-only name string or existing net to cast. `Power` and `Ground` also
accept `voltage`. `signal` declares the net's switching/sensitivity class
(`SignalType` from the prelude, or one of the strings `"static"`, `"digital"`,
`"clock"`, `"high_speed"`, `"switching_power"`, `"analog"`, `"rf"`); it
overrides any io()-declared class and drives routing analysis (crosstalk
aggressor/victim classification, EMI exposure) without net-name heuristics.
...
**Signature:** `io(name, typ_or_template, checks=None, optional=False, help=None, direction=None, sink_current=None, source_current=None, signal=None, matched_group=None)` or `io(typ_or_template, checks=None, optional=False, help=None, direction=None, sink_current=None, source_current=None, signal=None, matched_group=None)`
...
- `sink_current`: Optional maximum current this module draws through the connection (e.g. `"300mA"` or `Current("300mA")`). Net-typed io only.
- `source_current`: Optional maximum current this module can supply through the connection. Net-typed io only.
- `signal`: Optional switching/sensitivity class this module emits on the connection, one of `"static"`, `"digital"`, `"clock"`, `"high_speed"`, `"switching_power"`, `"analog"`, `"rf"`. Most meaningful with `direction="output"`. Net-typed io only.
- `matched_group`: Optional length-matching group name. Nets whose routed lengths must match (bus lanes, memory byte groups) declare the same group string; layout scoring reports the worst intra-group skew. A net belongs to at most one group — an explicit `Net(..., matched_group=...)` wins over io() declarations, and conflicting io() declarations are surfaced as a `matched_group_conflict` net property instead of picking one. Net-typed io only.
...
#### Current budgets and signal classes

`sink_current` and `source_current` declare, per connection, how much current a
module draws or can supply. The toolchain aggregates them per net into the
`current_sink_total` / `current_source_total` / `current_ports` net properties
and runs two ERC checks:

- `erc.current_budget` (error): a net's summed `source_current` cannot cover
  its summed `sink_current`.
- `erc.current_budget.undeclared` (warning): once a design declares currents
  anywhere, any net with no declared or inferable current is flagged. Designs
  with zero current declarations stay silent. Ground nets are exempt.

#### Static current inference through passives

Typed two-pin passives link nets with a known DC element model: resistors
contribute `1/R`, inductors and ferrite beads behave as near-shorts, and
capacitors and TVS diodes are open at DC (`1/jCw` with `w = 0`), so their
static current is known to be zero. Diodes, rectifiers and LEDs conduct only
anode-to-cathode above their `forward_voltage` (defaults: 0.7V, LEDs 2V), and
Zeners additionally conduct cathode-to-anode above their `zener_voltage`;
orientation comes from the component's SPICE net order (`nets=[A, K]`), so
any part with a SpiceModel is handled generically. Combined with declared net
voltages (`Power(voltage=...)`, `Ground` at 0V) this lets the ERC solve
source/impedance structures without hand-written current declarations — a
voltage divider or a Zener clamp from a declared rail reports its exact
static draw. Nonlinear elements are resolved by piecewise-linear state
iteration (off / forward / breakdown). Declared `sink_current` values act as
worst-case draws in the solve; declared `source_current` values are supply
capacities, never flows.

`pcb sim <file.zen> --static` generates the equivalent SPICE testbench from
the same declarations — every declared-voltage net becomes a DC source, every
declared sink a DC load, instantiated against the components' real SPICE
models with a `.op` analysis — so the built-in inference can be cross-checked
in ngspice (`--netlist` prints the deck without running it).

The inferred values land on nets as `current_sink_static` /
`current_source_static` and count toward the budget check
(`erc.current_budget`). When a resistive subnetwork cannot be solved — no
declared voltage reference, or a rail that sources current without declaring
its voltage — its nets get the `erc.current_budget.uninferable` warning.

`signal` classifies what the driving module emits. Per net, an explicit
`Net(..., signal=...)` declaration wins; otherwise agreeing io() declarations
apply, and disagreeing ones raise the `erc.signal_conflict` warning. Nets whose
type is `Power`/`Ground` are implicitly `static`; nets with an impedance target
are treated as at least `high_speed` by analysis passes. Many stdlib interfaces
pre-declare classes on their nets (e.g. `Spi.CLK` is `clock`).

```python
VBUS_IN = io(Power, sink_current="500mA")
VOUT = io(Power, direction="output", source_current="2A")
SW = io(Net, direction="output", signal="switching_power")
VSENSE = io(Net, signal="analog")
```
