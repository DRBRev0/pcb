...
the solver may fall back from. A pin name answers for one component, so a
constrained net that reaches two solves is an error rather than a race: give
each side its own `at()`.
