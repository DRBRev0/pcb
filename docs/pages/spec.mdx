...
capacitors are open at DC (`1/jCw` with `w = 0`), so their static current is
known to be zero. Combined with declared net voltages (`Power(voltage=...)`,
`Ground` at 0V) this lets the ERC solve simple resistive structures without
hand-written current declarations — a voltage divider from a declared 5V rail
reports its exact static draw on the rail. Declared `sink_current` values act
as worst-case draws in the solve; declared `source_current` values are supply
